...
function QuestionCreateForm({ questions }) {
  ...
  const { updateQuestion, getQuestions, setActive, createQuestionWithAnswer } =
    useQuestions();
      ...
      <form className="px-10" onSubmit={onSubmit}>
        ...
        {inputFields.map((inputField, index) => (
            ...
            <div key={index} className="relative w-full ">
              ...
              />
              <input type="checkbox" className="z-10 absolute top-0  h-full" onChange={(e) => onCheck(index, e)} checked={inputField.done} name="done" id="" />
              <button
                onClick={(e) => {
                  e.preventDefault();
                  removeFields(index);
